...
var aText = new Array(
    `${user}!!`,
    `Я уверен, что у тебя все хорошо, потому что я буду ${user} в будущем, я все знаю!`,
...
typingtext.addEventListener('mouseenter', (event) => {
    ...
    if(iRow === aText.length - 1) {
        ...
        typingtext.innerText += `여보세요?? 안녕!? 난 2070년의 미래의 너야!
    
        넌 틀림없이 잘 지내고 있을 거야 난 미래의 ${user}이니까 난 다 알아!

        이 말이 하고 싶어서 기다리느라 얼마나 힘들었는지 몰라! 네가 이 공중전화를 지나갈 때까지 기다렸어! 2070년에 이날을 기다리며 공중전화에서 바들바들 떨고 있었던 날 아니?
